...
const MenuList = () => {
  ...
  // Fetch menus (categories) dynamically from the backend
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetchMenuItems();
  }, [activeCategory, menus]); // Re-run when activeCategory or menus change
      ...
      >
        {menus.length > 0 ? (
          menus.map((menu) => (
            <div
              key={menu._id}
              onClick={() => setActiveCategory(menu.name)} // Set active category by name
              style={{
                width: "114px",
                height: "50px",
                backgroundColor:
                  activeCategory === menu.name ? "#0796EF" : "black", // Active category style
                display: "flex",
